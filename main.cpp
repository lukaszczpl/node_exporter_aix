...
int usage(int c) {
  ...
  std::cout << "  -p <port> TCP port to listen on, defaults to 9100"
            << std::endl;
  std::cout << std::endl;
  std::cout << "  -C Compat statistics, emulate linux node_exporter"
            << std::endl;
  ...
  std::cout << "  -L MPIO path statistics" << std::endl;
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt(argc, argv, "p:CcADPMmdiabpfvFLh?")) != EOF) {
    switch (c) {
    case 'p':
    ...
    case 'c':
      ...
      break;
    case 'A':
      flags |= PART_DISKADAPTER;
      break;
    case 'D':
      flags |= PART_DISKPATH;
      break;
    case 'M':
      flags |= PART_MEM_PAGES;
      break;
    case 'm':
      flags |= PART_MEM;
      break;
    case 'd':
      ...
      break;
    case 'i':
      flags |= PART_NETINTERFACE;
      break;
    case 'a':
      flags |= PART_NETADAPTER;
      break;
    case 'b':
      flags |= PART_NETBUFFER;
      break;
    case 'P':
      flags |= PART_PARTITION;
      break;
    case 'f':
      flags |= PART_FILESYSTEMS;
      break;
    case 'v':
      flags |= PART_VMSTAT_V;
      break;
  ...
  std::cout << "Node exporter for AIX version " PROG_VERSION
               " listening on port "
            << port << std::endl;
